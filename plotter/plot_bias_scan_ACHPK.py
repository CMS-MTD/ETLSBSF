...
def get_min_amp(run,ch):
	...
	if run>=53487 and run<=53633: minAmp=40
	if run>=53499 and run<=53525 and ch==4: minAmp=100
	if run>=53613 and run<=53633 and ch==4: minAmp=100
	if run>=53526 and run<=53612 and ch==4: minAmp=70
	if run>=53487 and run<=53536 and ch==6: minAmp=75
	if run>=53571 and run<=53633 and ch==6: minAmp=75
...
def plot_overlay(outfile,names,temps,series_num,plottype):
	...
	if "FBKprerad" in series_num: latex.DrawLatexNDC(0.64,0.91,"FBK UFSD4 2x2, +20 C")
...
def get_scan_results(scan_num,chan,laser,sensor_name):
	...
	with open(scan_txt_filename) as scan_txt_file:
		for line in scan_txt_file:
			...
			if scan_num==20011 or scan_num==20020: biases[-1] = biases[-1]-40
	...
	for i,run in enumerate(runs):
		...
		if type(run) is list:
			...
			for r in run:
				...
				counter = counter +1
				#if counter>=4: break
				if quick_mode and tree.GetEntries()>0: break
			run = run[0]
		else:
