...
def plot_overlay(outfile,names,temps,series_num,plottype):
	...
	mgraph = ROOT.TMultiGraph()
	if series_num=="HPK2_8e14_1p5e15":# or series_num=="CMSATLAS":
		...
		if showtemp:
			...
			leg = ROOT.TLegend(0.64,0.58,0.87,0.86)
	# if series_num=="TB":
	# 	leg = ROOT.TLegend(0.17,0.56,0.50,0.86)
	leg.SetMargin(0.15)

	if series_num=="TBCold":
		if plottype==16:
			leg=ROOT.TLegend(0.47,0.57,0.87,0.86)
		elif plottype==14:
			leg=ROOT.TLegend(0.17,0.57,0.57,0.86)

	if series_num=="CMSATLAS" or series_num=="CMS":
		if not square_mode:
			if series_num=="CMSATLAS":
				leg = ROOT.TLegend(0.15,0.6,0.54,0.84)
				leg.SetTextSize(0.028)
				leg2 = ROOT.TLegend(0.55,0.54,0.91,0.84)
				leg2.SetTextSize(0.028)
				leg2.SetNColumns(2)
			else:
				leg = ROOT.TLegend(0.15,0.55,0.66,0.84)
				leg.SetTextSize(0.035)
				leg.SetNColumns(3)
		else:
			leg = ROOT.TLegend(0.15,0.68,0.54,0.85)
			leg.SetTextSize(0.0195)
			leg.SetNColumns(3)
			leg2 = ROOT.TLegend(0.55,0.62,0.91,0.85)
			leg2.SetTextSize(0.0195)
			leg2.SetNColumns(2)	

	if series_num=="ATLAS":
		if not square_mode:
			leg = ROOT.TLegend(0.15,0.49,0.62,0.84)
			leg.SetTextSize(0.035) #28
			leg.SetNColumns(2)
		else:
			leg = ROOT.TLegend(0.15,0.61,0.55,0.84)
			leg.SetTextSize(0.0195)
			leg.SetNColumns(3)
	...
	for i,scan in enumerate(scan_nums):
		...
		if showtemp:
			...
			if series_num=="CMSATLAS" and "ATLAS" in names[i]: leg2.AddEntry(graph, "%s" %(names[i].replace("CMS ","").replace("ATLAS ","").replace("MW","metal").replace("metal","(metal)")),"EP")
			else: 
				leg.AddEntry(graph, "%s" %(names[i].replace("CMS ","").replace("ATLAS ","").replace("MW","metal").replace("metal","(metal)")),"EP")
	...
	if series_num=="HPK2_8e14_1p5e15" or series_num=="CMSATLAS":
		currentymin = mgraph.GetYaxis().GetXmin()
		currentymax = mgraph.GetYaxis().GetXmax()
		mgraph.GetYaxis().SetRangeUser(currentymin, 1.7*currentymax)
	...
	elif y_axis == "Risetime (10 to 90%) [ps]":
		mgraph.GetYaxis().SetRangeUser(400,750)
		if "charge" in x_axis:
			mgraph.GetXaxis().SetLimits(0,60)
			mgraph.GetXaxis().SetRangeUser(0,60)
		if series_num == "TB": mgraph.GetYaxis().SetRangeUser(350,850)
		if series_num == "2": mgraph.GetYaxis().SetRangeUser(400,650)
	elif (plottype==16 or plottype==17) and ("ATLAS" in series_num or "CMS" in series_num): 
		mgraph.GetYaxis().SetRangeUser(20,80)
		mgraph.GetXaxis().SetLimits(0,60)
		mgraph.GetXaxis().SetRangeUser(0,60)
	elif plottype==14:
	 	if series_num=="CMS": mgraph.GetYaxis().SetRangeUser(0,45)
	 	if series_num=="ATLAS": mgraph.GetYaxis().SetRangeUser(0,60)
	 	if series_num=="TBCold": mgraph.GetYaxis().SetRangeUser(0,30)
	 	if series_num=="2": mgraph.GetYaxis().SetRangeUser(0,35)
	...
	if series_num=="CMSATLAS": leg2.Draw()
	latex= ROOT.TLatex()
	...
	if "CMS" in series_num: latex.DrawLatexNDC(labelx1,labely,"1x3 mm^{2} pads")
	if "CMSATLAS" in series_num: latex.DrawLatexNDC(labelx2,labely,"1.3x1.3 mm^{2} pads")
	elif "ATLAS" in series_num: latex.DrawLatexNDC(labelx1,labely,"1.3x1.3 mm^{2} pads")
	latex.SetTextSize(0.05)
	if square_mode: latex.SetTextSize(0.035)
	latex.SetTextFont(42)
	if "TB" not in series_num: latex.DrawLatexNDC(0.15,0.91,"^{106}Ru #beta^{-} source")
	if series_num == "2": latex.DrawLatexNDC(0.64,0.91,"HPK type 3.1, +20 C")
	elif series_num=="TBCold": 
		latex.DrawLatexNDC(0.15,0.91,"HPK 3.1, 1x3 mm^{2} P2, 50 #mum IP")
		latex.DrawLatexNDC(0.84,0.91,"-20 C")

	else: latex.DrawLatexNDC(0.64,0.91,"HPK type 3.1, -20 C")
	# latex.DrawLatexNDC(0.65,0.91,"HPK type 3.1, -20 C")
	c.Print("plots/series%s%s.pdf"%(series_num,outputtag))
	c.Print("plots/series%s%s.root"%(series_num,outputtag))
...
def cosmetic_tgraph_organized(graph,sensorname,tb=False):
	...
	if "CMS" in sensorname:
		...
		if "MW" in sensorname or "metal" in sensorname:
			markerstyle=21
		elif "50x500" in sensorname or "50 #mum" in sensorname or "IP50" in sensorname:
			markerstyle=23
	elif "ATLAS" in sensorname:
		linestyle=7
		if "90 #mum metal" in sensorname or ("IP90" in sensorname and "metal" in sensorname): colorindex=0
		elif "90 #mum" in sensorname or "IP90" in sensorname: colorindex=1
		elif "50 #mum" in sensorname or "IP50" in sensorname: colorindex=5
		elif "30 #mum" in sensorname or "IP30" in sensorname: colorindex=4
...
def get_risetime_channel(tree,ch,run=-1):
	hist = ROOT.TH1D("h","",60,100,1500)
	minAmp = get_min_amp(run)
	hist.StatOverflows(False)
	#10 to 90 risetime
	tree.Project("h","1e12*abs(0.8*amp[%i]/risetime[%i])"%(ch,ch),"amp[%i]>%i && amp[%i]<360"%(ch,minAmp,ch))	### mV/ s
...
def get_charge_channel(tree,ch,run=-1,laser=False):
	...
	elif run==151592: hist = ROOT.TH1D(histname,"",50,2,200)
